...
``modprobe.blacklist=amdgpu``
  ...
  For some system configurations, the ``amdgpu`` driver needs to be blocklisted to avoid an issue where:

  After boot, GPUs are not listed when running the command ``rocm-smi`` or ``amd-smi``.

  Alternatively, setting AMD recommended System Optimized BIOS settings may remove
...
After this change, the ``amdgpu`` module must be loaded to support the ROCm framework
